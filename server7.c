...
int main(int argc, char *argv[])
{
    ...
    // клиент для логирования
    struct sockaddr_in client_log;
    int name_log[2] = {-1, 0};
    recvfrom(sockfd, NULL, 0, 0, (struct sockaddr *)&client_log, &addr_len);
    sendto(sockfd, name_log, sizeof(name_log), 0, (struct sockaddr *)&client_log, addr_len);
    ...
    for (int i = 0; i < cnt_proc; ++i) {
        ...
        char receivedLog[256]; // ждём сигнала конца диалога
        recvfrom(sockfd, &receivedLog, sizeof(receivedLog), 0, (struct sockaddr *)&client_addrs[i], &addr_len);
        sendto(sockfd, receivedLog, sizeof(receivedLog), 0, (struct sockaddr *)&client_log, addr_len);
    }
        ...
        if (sons[i] < 0) {
            ...
            while (1) {
                if (state) { // проверка на звонящего
                    ...
                    char log_inf_who[256];
                    snprintf(log_inf_who, sizeof(log_inf_who), "%d %s", i, "назначен звонящим\n");
                    sendto(sockfd, log_inf_who, sizeof(log_inf_who), 0, (struct sockaddr *)&client_log, addr_len);
                    ...
                    printf("%d %s", i, "звонит\n");
                    char log_inf[256];
                    snprintf(log_inf, sizeof(log_inf), "%d %s", i, "звонит\n");
                    sendto(sockfd, log_inf, sizeof(log_inf), 0, (struct sockaddr *)&client_log, addr_len);
                    ...
                    char log_inf_who[256];
                    snprintf(log_inf_who, sizeof(log_inf_who), "%d %s", i, "назначен ждущим\n");
                    sendto(sockfd, log_inf_who, sizeof(log_inf_who), 0, (struct sockaddr *)&client_log, addr_len);
                    int own_value;
                    while (1) { // процесс ожидания
                        ...
                        if (own_value == 0) {
                            ...
                            printf("%d %s", i, "принял\n");
                            char log_inf[256];
                            snprintf(log_inf, sizeof(log_inf), "%d %s", i, "принял\n");
                            sendto(sockfd, log_inf, sizeof(log_inf), 0, (struct sockaddr *)&client_log, addr_len);
                            sendto(sockfd, nums, sizeof(nums), 0, (struct sockaddr *)&client_addr, addr_len); // начал разговор
                            break;
                        }
                ...
                char receivedLog[256]; // ждём сигнала конца диалога
                recvfrom(sockfd, &receivedLog, sizeof(receivedLog), 0, (struct sockaddr *)&client_addr, &addr_len);
                sendto(sockfd, receivedLog, sizeof(receivedLog), 0, (struct sockaddr *)&client_log, addr_len);
                char receivedLog2[256]; // ждём сигнала конца диалога
                recvfrom(sockfd, &receivedLog2, sizeof(receivedLog2), 0, (struct sockaddr *)&client_addr, &addr_len);
                sendto(sockfd, receivedLog2, sizeof(receivedLog2), 0, (struct sockaddr *)&client_log, addr_len);
                // обновляем роль
                state = rand() % 2;
                if (!state) { // проверка на ждущего
                    sem_post(sem[i]);
